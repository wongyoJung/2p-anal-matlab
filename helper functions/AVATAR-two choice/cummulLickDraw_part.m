function [cummulLick,countA,countB] = cummulLickDraw_part(directory,filename,animalID)
...
for t=15*60:3:30*60
    ...
    LickA=length(find(strcmp(select(startID(end):TimeId,1),'LickA')));
    LickB=length(find(strcmp(select(startID(end):TimeId,1),'LickB')));
