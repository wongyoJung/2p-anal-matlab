...
boutEnds = adjusted(find(diff(adjusted)>1*FL));
boutStarts = adjusted(find(diff(adjusted)>1*FL)+1);
...
% baseline = mean(Lfilter(1:boutStarts(1)));

    lick = boutendidx - boutstartidx;
    
%     if((boutend-boutstart)>3*FL)
      if(lick>2)
        licks(length(licks)+1) = lick;
        dF = (Lfilter(boutstart:boutstart+3*FL)-mean(baseline))/std(baseline);
